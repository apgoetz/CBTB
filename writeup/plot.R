...
plot(ecdf(sdata[[1]]), main="", xlab="Stack Depth",ylab="ECDF",  verticals=T, pch=46)
dev.off()

pdf("ecdf-fp-1-stack.pdf")
fpdata <- read.table('datasets/fp-1-stack-depth.gz')
plot(ecdf(fpdata[[1]]), main="", xlab="Stack Depth",ylab='ECDF', verticals=T, pch=46)
dev.off()
...
plot(ecdf(ddata[[1]]), main="", xlab="Jump Displacement", ylab='ECDF',verticals=T, pch=46)
dev.off()
